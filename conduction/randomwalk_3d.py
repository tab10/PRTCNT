...
def serial_method(grid_size, tube_length, tube_radius, num_tubes, orientation, tot_time, quiet, plot_save_dir,
    ...
    prev_type_hot = 0  # assume walker was in matrix before 1st step. this tells the previous type of
    ...
    for i in range(tot_time):
        ...
        for j in range(len(hot_walker_master) - trigger):  # except the new ones
            ...
            current_hot_updated, prev_type_hot = rules_3d.apply_moves_3d(hot_temp, kapitza, grid, prob_m_cn,
                                                                         prev_type_hot)
            current_hot_updated.erase_prev_pos()
            hot_walker_master[j] = current_hot_updated

            cold_temp = cold_walker_master[j]
            current_cold_updated, prev_type_cold = rules_3d.apply_moves_3d(cold_temp, kapitza, grid, prob_m_cn,
                                                                           prev_type_cold)
...
def parallel_method(grid_size, tube_length, tube_radius, num_tubes, orientation, tot_time, quiet, plot_save_dir,
    ...
    if rank == 0:
        if gen_plots:
            plots.plot_three_d_random_walk_setup(grid, quiet, plot_save_dir, inert_vol)
    else:
        grid = None
    ...
    for i in range(walkers_per_core_whole):
        ...
        for j in range(walkers_per_timestep):
            ...
            if save_loc_plots:
                plots.plot_walker_path_3d_onlat(hot_temp, grid_size, 'hot', quiet, i, plot_save_dir)
                plots.plot_walker_path_3d_onlat(cold_temp, grid_size, 'cold', quiet, i, plot_save_dir)
